...
const RecipeDetailScreen = () => {
  ...
  useEffect(() => {
    ...
    const loadSavedProgress = async () => {
      try {
        const savedIngredients = await SecureStore.getItemAsync(`recipe_${recipeId}_ingredients`);
        const savedInstructions = await SecureStore.getItemAsync(`recipe_${recipeId}_instructions`);
        
        if (savedIngredients) {
          setCompletedIngredients(JSON.parse(savedIngredients));
        }
        if (savedInstructions) {
          setCompletedInstructions(JSON.parse(savedInstructions));
        }
      } catch (error) {
  ...
  const toggleIngredient = async (index) => {
    const updatedIngredients = completedIngredients.includes(index)
      ? completedIngredients.filter(i => i !== index)
      : [...completedIngredients, index];

    setCompletedIngredients(updatedIngredients);

    await SecureStore.setItemAsync(
      `recipe_${recipeId}_ingredients`,
      JSON.stringify(updatedIngredients)
    );
  };

  const toggleInstruction = async (index) => {
    const updatedInstructions = completedInstructions.includes(index)
      ? completedInstructions.filter(i => i !== index)
      : [...completedInstructions, index];

    setCompletedInstructions(updatedInstructions);

    await SecureStore.setItemAsync(
      `recipe_${recipeId}_instructions`,
  ...
  const resetProgress = async () => {
    Alert.alert(
      "Reset Progress",
      "Are you sure you want to reset all progress for this recipe? This cannot be undone.",
      [
        {
          text: "Cancel",
          style: "cancel",
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              // Clear storage
              await SecureStore.deleteItemAsync(`recipe_${recipeId}_ingredients`);
              await SecureStore.deleteItemAsync(`recipe_${recipeId}_instructions`);
    ...
    <View style={recipeDetailStyles.container}>
      <ScrollView>
        ...
        <View style={recipeDetailStyles.contentSection}>
          ...
          <View style={recipeDetailStyles.sectionContainer}>
            ...
            <View style={recipeDetailStyles.ingredientsGrid}>
              {recipe.ingredients.map((ingredient, index) => {
                const isCompleted = completedIngredients.includes(index);
                return (
                  <TouchableOpacity 
                    key={index} 
                    style={[
                      recipeDetailStyles.ingredientCard,
                      isCompleted && { backgroundColor: COLORS.primary + '10', borderColor: COLORS.primary }
                    ]}
                  ...
                  >
                    ...
                    <Text style={[
                      ...
                      isCompleted && { textDecorationLine: 'line-through', color: COLORS.textLight }
                    ]}>
                      {ingredient}
                    </Text>
              ...
              {recipe.instructions.map((instruction, index) => {
                ...
                return (
                  <View key={index} style={[
                    ...
                    <LinearGradient
                      colors={isCompleted ? [COLORS.primary, COLORS.primary] : [COLORS.primary, COLORS.primary + "CC"]}
                      ...
                      <View style={recipeDetailStyles.instructionFooter}>
                        ...
                        <TouchableOpacity 
                          style={[
                            ...
                            isCompleted && { backgroundColor: COLORS.primary + '20' }
                          ]}
                        ...
                        >
                          <Ionicons 
                            ...
                            size={16} 
                            color={isCompleted ? COLORS.primary : COLORS.primary} 
                          />
                        </TouchableOpacity>
                      </View>
                    </View>
          ...
          <TouchableOpacity
            style={[recipeDetailStyles.primaryButton, { marginTop: 12 }]}
